...
class TaskWithTags {
  Task task;
  String? tagNames;                // Space-separated names from SQL GROUP_CONCAT
...
Future<Task>       updateTask(String taskId, {required String title, DateTime? dueDate, bool isAllDay = true, String? notes, String? notificationType});
...
| Task highlighted | `TaskProvider.navigateToTask()` | Brief visual highlight on task item |
...
```dart
class TagColors {
  static const Color defaultColor;               // Blue 500 (#2196F3)
  static const List<Color> presetColors;          // 12 Material Design colors
  static const Map<String, Color> textColorMap;   // Hex → text color for WCAG AA
