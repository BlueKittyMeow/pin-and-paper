# Phase 3.6A Plan: Tag Filtering (v3.1)

**Version:** 3.1 (production-ready - all blocking bugs fixed)
**Created:** 2026-01-09
**Updated:** 2026-01-09 (v3 sanity check, applied 15 fixes)
...
## Changes from v3 (Pre-implementation Sanity Check)

**Blocking bugs fixed (3 HIGH):**
1. 🔴 **H1:** Fixed compilation error - `const FilterState()` → `FilterState.empty`
2. 🔴 **H2:** Fixed race condition in error rollback - guard with operation ID check
3. 🔴 **H3:** Integrated empty results state widget into TaskListScreen

**Consistency & UX fixes (5 MEDIUM):**
4. 🟡 **M1:** Factory returns `FilterState.empty` for default params (optimization)
5. 🟡 **M2:** Changed addTagFilter to use TagProvider instead of DB query (per recommendation)
6. 🟡 **M3:** Added `completed` parameter to TagFilterDialog (show correct counts)
7. 🟡 **M4:** Added "Clear All" button to TagFilterDialog (UX improvement)
8. 🟡 **M5:** Added empty state to TagFilterDialog when no tags exist

**Robustness & polish (7 LOW):**
9. 🟢 **L1:** Added error handling to fromJson (defensive programming)
10. 🟢 **L2:** Optimized ghost tag filtering from O(n*m) to O(n+m)
11. 🟢 **L3:** Optimized performance test setup with batch inserts
12. 🟢 **L4:** Documented haptic feedback platform support
13. 🟢 **L5:** Pass TagService as constructor parameter (cleaner architecture)
14. 🟢 **L6:** Added database schema verification step to Day 1
15. 🟢 **L7-L8:** Added missing concurrency and edge case integration tests

**See:**
- `docs/phase-3.6A/claude-findings-v3-sanity-check.md` - Full analysis (18 issues found)
- `docs/phase-3.6A/v3-to-v3.1-fix-plan.md` - Detailed fix specifications
...
class TaskProvider extends ChangeNotifier {
  ...
  // NEW: Filter state
  // H1: Use FilterState.empty instead of const FilterState()
  ...
  Future<void> setFilter(FilterState filter) async {
    ...
    try {
      ...
      // FIX #4 + H2: Rollback to previous filter state on error
  ...
  /// H1: Use FilterState.empty (factory constructors can't be const)
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // H3: Show empty state when no tasks match filters
          Expanded(
